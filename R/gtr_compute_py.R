#' GTR Compute Function using python
#'
...
gtr_compute_py <- function(TYX,tYX,y,x,
  ...
  while(length(cvg)==0 && tol.res*fac.now < 10 && length(res0)>0 && !exit){ # until cvg converges (to 1)
    ...
    try(res0 <- gtr_solve_py(TYX=TYX,tYX=tYX))
      ...
      if(gtest$p.value<.05){
        if(length(pen)==0){
          ...
          if(!is.na(res0$llf)){
            ...
            res.dual <- NULL
            try(res.dual <- gtr_solve_py(TYX=TYX,tYX=tYX))
          ...
          res.alg2 <- NULL
          ifelse(algor=="ECOS", algor<-"SCS", algor<-"ECOS"); ifelse(algor=="SCS", tol.res <- 1.e-01, tol.res <- 1e-4);
          try(res.alg2 <- gtr_solve_py(TYX=TYX,tYX=tYX))
