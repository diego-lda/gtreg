...
gtr_al_py <- function(y,x,X.type=NULL,gam=0,lam.vec=rep(1,6),xdf=0,ydf=4,yorder=4,nyg.max=101,Ysing=FALSE,y_knots=NULL,ng.qgm=101,
                   ...
                   delta.ok=F,parallel=F,eta.check=T){
  ...
  if(length(model)==0){
    ...
    if(dim(x)[2] == 1 && all(x[,1] == 1)){
      datmat     <- data_prep(y=as.numeric(y),x=x,ygrid=ygrid.qgm,
                              Xs=x,nxgrid=0,
                              y_knots=y_knots,ydf=ydf,addxint=addxint,yorder=yorder,
                              yorth=yorth,xorth=xorth,Ysing=Ysing,e0mode=e0mode,nygrid=ng.qgm)
    }else{
      datmat     <- data_prep(y=as.numeric(y),x=x,ygrid=ygrid.qgm,xgrid=xgrid.qgm,
                              info=info,y_knots=y_knots,ydf=ydf,addxint=addxint,yorder=yorder,
    ...
    while( dedy.min <= .Machine$double.eps && nyg < nyg.max ){
      #while( dedy.min <= cval && nyg < nyg.max ){

      if(nyg>201 && easy==F && xdf !=0){ easy <- T; nyg <- 1}
      nyg     <- 2*nyg + 1
      if(nyg==1){ nyg <- 3 }; cat(paste("nyg now =",nyg,"\n"))
      if(nyg==0){
        Xsgrid <- Xs; #rm(Xs);
        sYgrid <- NULL
      ...
      if(nyg>0){
        ...
        if(dim(x)[2] == 1 && all(x[,1] == 1) ){
          datmat <- data_prep(y=as.numeric(y),x=x,ygrid=ygrid,
                              Xs=x,nxgrid=0,
                              y_knots=y_knots,ydf=ydf,addxint=addxint,yorder=yorder,
                              yorth=yorth,xorth=xorth,Ysing=Ysing,e0mode=e0mode)
        }else{
          datmat <- data_prep(y=as.numeric(y),x=x,ygrid=ygrid,xgrid=xgrid,
                              info=info,y_knots=y_knots,ydf=ydf,addxint=addxint,yorder=yorder,
      ...
      ans <- gtr_compute_py(nyg=nyg,TYX=TYX,tYX=tYX,gam=gam,reltol=reltol,feastol=feastol,abstol=abstol,
                          ...
                          tol.res=reltol,algor=algor,easy=easy,e0mode=e0mode,eta.check=eta.check )
    ...
    if(dim(x)[2] == 1 && all(x[,1] == 1)){
      datmat.now <- data_prep(y=as.numeric(y),x=x, y_knots=y_knots,ydf=ydf, Xs=x,nxgrid=0,
                              addxint=addxint,yorder=yorder,yorth=FALSE,xorth=FALSE,
                              Ysing=Ysing,e0mode=e0mode,returnTZ=T,returnTZg=F)
    }
    else{
      datmat.now <- data_prep(y=as.numeric(y),x=x, info=info,y_knots=y_knots,ydf=ydf,
  ...
  if( AL && min(res.sol$eta)>0 ){
    ...
    datmat     <- data_prep(y=as.numeric(y),x=x,ygrid=ygrid.qgm,xgrid=xgrid.qgm,bmat=matrix(res.sol$bmat,nr=nXs,nc=nYS),
                            info=info,y_knots=y_knots,ydf=ydf,addxint=addxint,yorder=yorder,
                            yorth=yorth,xorth=xorth,Ysing=Ysing,e0mode=e0mode,nxgrid=ng.qgm,nygrid=ng.qgm)
    TYX        <- datmat$TZ;
    tYX        <- datmat$tZ
    ...
    for(gg in 1:nglam){
      ...
      while( dedy.min <= .Machine$double.eps && nyg < nyg.max ){
        ...
        ans     <- gtr_compute_py( nyg=nyg,TYX=TYXnow,tYX=tYXnow,gam=gam.grid[gg],weights=weights,zeros=zeros,
                                ...
                                Xs.qgm=Xsgrid.qgm,sYgrid.qgm=sYgrid.qgm,
                                maxit=maxit,ng.qgm=ng.qgm,dedy.min=dedy.min,
                                tol.res=reltol,algor=algor,easy=easy,threshold=threshold,e0mode=e0mode,eta.check=eta.check )
    ...
    select.bic <- mstar.fun(round(objval.bic,digits=1))
  ...
  ifelse(parallel, ans    <- list(BIC=BIC.vec, bmat=res.sol$bmat),
